-- Same reasoning as patch_002 for cart_item: order_item needs its own
-- denormalized image snapshot so Order History/Detail can render without a
-- join back through product/variant data (which may have changed since).

alter table order_item add column if not exists image_url text;

-- Re-create create_order to also populate the new column (CREATE OR REPLACE
-- of the exact same function from schema.sql, just with one added field).
...
begin
  ...
  v_order_number := 'A11-' || upper(substr(encode(gen_random_bytes(5), 'hex'), 1, 8));
